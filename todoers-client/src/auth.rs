...
pub fn register_begin(
    username: &str,
    password: &str,
) -> TodoersResult<(RegisterFlow, StartRegisterRequest)> {
    let mut rng = OsRng;
...
) -> TodoersResult<(FinishRegisterRequest, NewAccount)> {
    let mut rng = OsRng;
...
pub fn login_begin(
    username: &str,
    password: &str,
) -> TodoersResult<(LoginFlow, LoginStartRequest)> {
    let mut rng = OsRng;
...
/// wrong password fails here with `TodoersError::Opaque`.
#[tracing::instrument(skip(flow, response))]
pub fn login_finish(
    flow: LoginFlow,
    response: LoginStartResponse,
) -> TodoersResult<(LoginFinishRequest, Zeroizing<Vec<u8>>)> {
    let mut rng = OsRng;
...
pub fn unlock_from_device_cache(
    ...
    blob: &[u8],
) -> TodoersResult<Zeroizing<DeviceCachePayload>> {
    let mut json = crypto::device_open(identity_contents, blob)?;
    let payload: DeviceCachePayload =
        serde_json::from_slice(&json).map_err(|_| TodoersError::Aead)?;
    json.zeroize();
