...
impl TodoDoc {
    ...
    pub fn from_snapshot(bytes: &[u8]) -> TodoersResult<Self> {
        let doc = LoroDoc::new();
        doc.import(bytes)?;
        Ok(Self { doc })
    }
    ...
    pub fn export_snapshot(&self) -> TodoersResult<Vec<u8>> {
        self.doc.commit();
    ...
    pub fn export_updates_from(&self, from: &VersionVector) -> TodoersResult<Vec<u8>> {
        self.doc.commit();
    ...
    pub fn set_title(&self, title: &str) -> TodoersResult<()> {
    ...
    pub fn add_item(&self, input: &TodoItemInput) -> TodoersResult<String> {
    ...
    pub fn update_item(&self, id: &str, input: &TodoItemInput) -> TodoersResult<()> {
        let map = self.item_map(id).ok_or(TodoersError::UnknownId)?;
        map.insert("title", input.title.as_str())?;
        map.insert("notes", input.notes.as_str())?;
        map.insert("priority", input.priority.rank())?;
        match input.due {
    ...
    pub fn toggle_done(&self, id: &str) -> TodoersResult<()> {
        let map = self.item_map(id).ok_or(TodoersError::UnknownId)?;
        let now = map
            .get("done")
    ...
    pub fn set_priority(&self, id: &str, priority: Priority) -> TodoersResult<()> {
        let map = self.item_map(id).ok_or(TodoersError::UnknownId)?;
    ...
    pub fn set_due(&self, id: &str, due: Option<OffsetDateTime>) -> TodoersResult<()> {
        let map = self.item_map(id).ok_or(TodoersError::UnknownId)?;
        match due {
    ...
    pub fn move_item(&self, from: usize, to: usize) -> TodoersResult<()> {
        self.items_list().mov(from, to)?;
        self.doc.commit();
        Ok(())
    }

    pub fn remove_item(&self, id: &str) -> TodoersResult<()> {
        let idx = self.item_index(id).ok_or(TodoersError::UnknownId)?;
    ...
    pub fn add_tag(&self, id: &str, tag: &str) -> TodoersResult<()> {
        let map = self.item_map(id).ok_or(TodoersError::UnknownId)?;
        let tags = map.get_or_create_container(TAGS, LoroList::new())?;
        tags.push(tag)?;
        self.doc.commit();
    ...
    pub fn remove_tag(&self, id: &str, tag: &str) -> TodoersResult<()> {
        let map = self.item_map(id).ok_or(TodoersError::UnknownId)?;
        let tags = map.get_or_create_container(TAGS, LoroList::new())?;
        for i in 0..tags.len() {
            let matches = tags
    ...
    pub fn add_subtask(&self, item_id: &str, title: &str) -> TodoersResult<String> {
        let map = self.item_map(item_id).ok_or(TodoersError::UnknownId)?;
    ...
    pub fn toggle_subtask(&self, item_id: &str, subtask_id: &str) -> TodoersResult<()> {
        let map = self.item_map(item_id).ok_or(TodoersError::UnknownId)?;
        let subs = map.get_or_create_container(SUBTASKS, LoroMovableList::new())?;
        for i in 0..subs.len() {
    ...
    pub fn remove_subtask(&self, item_id: &str, subtask_id: &str) -> TodoersResult<()> {
        let map = self.item_map(item_id).ok_or(TodoersError::UnknownId)?;
        let subs = map.get_or_create_container(SUBTASKS, LoroMovableList::new())?;
        for i in 0..subs.len() {
