...
--       nothing new.
--
--   (2) plaintext-for-convenience — safe at rest because this whole DB is
--       SQLCipher-encrypted. Decrypted list names, the Loro document, the todo
--       read model. The DB is opened with `PRAGMA key` set to a random 32-byte
--       SQLCipher key (NOT password-derived). That key is wrapped at
--       {data_dir}/db_keys.json for a device recipient (X-Wing; private key in
--       the OS keyring) and a recovery-key recipient, and is recovered before
--       the DB opens. See todoers-client/src/sqlcipher.rs and ENCRYPTION.md.
--
