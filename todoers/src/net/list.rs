...
pub async fn create_list(
    ...
    let resp: ListIdDto = Client::new()
        ...
        .send()
        .await
        .context("create list request failed")?
        .error_for_status()
        .context("create list rejected by the server")?
        .json()
        .await
        .context("invalid create list response")?;
    Ok(resp.list_id)
}

/// The ids of every list the caller is a member of.
pub async fn fetch_lists(base_url: &str, token: &str) -> anyhow::Result<ListIdsDto> {
    let base = base_url.trim_end_matches('/');
    let resp = Client::new()
        ...
        .send()
        .await
        .context("fetch lists request failed")?
        .error_for_status()
        .context("fetch lists rejected by the server")?;
    let lists: ListIdsDto = resp.json().await.context("unable to parse list ids")?;
    Ok(lists)
}
...
pub async fn get_metadata(
    ...
    let resp: MetadataResponse = Client::new()
        ...
        .send()
        .await
        .context("get metadata request failed")?
        .error_for_status()
        .context("get metadata rejected by the server")?
        .json()
        .await
        .context("invalid metadata response")?;
    Ok(resp)
}
...
pub async fn add_member(
    ...
    Client::new()
        ...
        .send()
        .await
        .context("add member request failed")?
        .error_for_status()
        .context("add member rejected by the server")?;
    Ok(())
}
...
pub async fn remove_member(
    ...
    Client::new()
        ...
        .send()
        .await
        .context("remove member request failed")?
        .error_for_status()
        .context("remove member rejected by the server")?;
    Ok(())
}
...
pub async fn get_my_keys(
    ...
    let resp: Vec<KeySlotDto> = Client::new()
        ...
        .send()
        .await
        .context("get keys request failed")?
        .error_for_status()
        .context("get keys rejected by the server")?
        .json()
        .await
        .context("invalid keys response")?;
    Ok(resp)
}
